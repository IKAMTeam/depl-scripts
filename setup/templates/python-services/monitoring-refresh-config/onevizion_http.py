...
class Singleton(object):
    """ Make sure this process is only running once.  It does a quiet quit() if it's already running.
        ...
        * May set custom Quit Message.
        ...
        http://github.com/pycontribs/tendo/blob/master/tendo/singleton.py
...
class Trackor(object):
    ...
    def delete(self,trackorId):
        ...
        URL = "http://{URL}/api/v3/trackor_types/{TrackorType}/trackors?{FilterSection}".format(URL=self.URL, TrackorType=self.TrackorType, FilterSection=FilterSection)
        self.errors = []
        self.jsonData = {}
    ...
    def read(self,
        ...
        URL = "http://{Website}/api/v3/trackor_types/{TrackorType}/trackors".format(
            Website=self.URL,
        ...
        if trackorId is None:
            ...
            URL = "http://{Website}/api/v3/trackors/{TrackorID}".format(
                Website=self.URL,
    ...
    def update(self, trackorId=None, filters={}, fields={}, parents={}, charset=""):
        ...
        if trackorId is None:
            ...
            URL = "http://{Website}/api/v3/trackor_types/{TrackorType}/trackors{Filter}".format(
                Website=self.URL,
                TrackorType=self.TrackorType,
                Filter=Filter
            )
    ...
    def create(self,fields={},parents={}, charset=""):
        ...
        URL = "http://{URL}/api/v3/trackor_types/{TrackorType}/trackors".format(URL=self.URL, TrackorType=self.TrackorType)
    ...
    def assignWorkplan(self,trackorId, workplanTemplate, name=None, startDate=None, finishDate=None):
        ...
        URL = "http://{website}/api/v3/trackors/{trackor_id}/assign_wp?workplan_template={workplan_template}".format(
            website=self.URL,
    ...
    def GetFile(self, trackorId, fieldName):
        ...
        URL = "http://{Website}/api/v3/trackor/{TrackorID}/file/{ConfigFieldName}".format(
            Website=self.URL,
            TrackorID=trackorId,
...
#http://trackor.onevizion.com/api/v3/trackor/{TrackorID}/file/{ConfigFieldName}?file_name={NewFileName}
...
class WorkPlan(object):
    ...
    def read(self, workplanId = None, workplanTemplate = "", trackorType = "", trackorId = None):
        ...
        URL = "http://{URL}/api/v3/wps/{FilterSection}".format(URL=self.URL, FilterSection=FilterSection)
        self.errors = []
        self.jsonData = {}
...
class Task(object):
    ...
    def read(self, taskId = None, workplanId=None, orderNumber=None):
        ...
        if taskId is not None:
            URL = "http://{URL}/api/v3/tasks/{TaskID}".format(URL=self.URL, TaskID=taskId)
        elif orderNumber is not None:
            URL = "http://{URL}/api/v3/tasks?workplan_id={WorkPlanID}&order_number={OrderNumber}".format(URL=self.URL, WorkPlanID=workplanId, OrderNumber=orderNumber)
        else:
            URL = "http://{URL}/api/v3/wps/{WorkPlanID}/tasks".format(URL=self.URL, WorkPlanID=workplanId)
...
class Import(object):
    ...
    def run(self):
        if self.isTokenAuth:
            ...
            self.auth = requests.auth.HTTPBasicAuth(self.userName, self.password)
        self.ImportURL = "http://{URL}/api/v3/imports/{ImpSpecID}/run?action={Action}".format(
            URL=self.URL,
            ImpSpecID=self.impSpecId,
            Action=self.action
        )
    ...
    def interrupt(self,ProcessID=None):
        ...
        if self.isTokenAuth:
            ...
            self.auth = requests.auth.HTTPBasicAuth(self.userName, self.password)
        self.ImportURL = "http://{URL}/api/v3/imports/runs/{ProcID}/interrupt".format(
            URL=self.URL,
            ProcID=PID
        )
    ...
    def getProcessData(self,
        ...
        if self.isTokenAuth:
            ...
            self.auth = requests.auth.HTTPBasicAuth(self.userName, self.password)
        self.ImportURL = "http://{URL}/api/v3/imports/runs".format(
            URL=self.URL
        )
...
class Export(object):
    ...
    def run(self):
        if self.isTokenAuth:
            ...
            self.auth = requests.auth.HTTPBasicAuth(self.userName, self.password)
        self.ImportURL = "http://{URL}/api/v3/exports/{TrackorType}/run?export_mode={ExportMode}&delivery={Delivery}".format(
            URL=self.URL,
            TrackorType=self.trackorType,
    ...
    def interrupt(self,ProcessID=None):
        ...
        if self.isTokenAuth:
            ...
            self.auth = requests.auth.HTTPBasicAuth(self.userName, self.password)
        self.ImportURL = "http://{URL}/api/v3/exports/runs/{ProcID}/interrupt".format(
            URL=self.URL,
            ProcID=PID
        )
        self.OVCall = curl('POST',self.ImportURL,auth=self.auth)
    ...
    def getProcessStatus(self,ProcessID=None):
        ...
        if self.isTokenAuth:
            ...
            self.auth = requests.auth.HTTPBasicAuth(self.userName, self.password)
        self.ImportURL = "http://{URL}/api/v3/exports/runs/{ProcID}".format(
            URL=self.URL,
            ProcID=PID
        )
        self.OVCall = curl('GET',self.ImportURL,auth=self.auth)
    ...
    def getFile(self,ProcessID=None):
        ...
        if self.isTokenAuth:
            ...
            self.auth = requests.auth.HTTPBasicAuth(self.userName, self.password)
        self.ImportURL = "http://{URL}/api/v3/exports/runs/{ProcID}/file".format(
            URL=self.URL,
            ProcID=PID
        )
...
if sys.version_info.major >= 3 and sys.version_info.minor >= 4:
    ...
    class NotifQueue:
        ...
        def getNotifQueue(self):
            URL = "http://{URL}/api/internal/notif/queue?service_id={ServiceID}".format(URL=self._URL, ServiceID=self._serviceId)
        ...
        def updateNotifQueueRecStatusById(self, notifQueueRecId, status):
            URL = "http://{URL}/api/internal/notif/queue/{notifQueueRecId}/update_status?status={status}".format(URL=self._URL, notifQueueRecId=notifQueueRecId, status=status)
            OVCall = curl('PATCH', URL, headers=self._headers, auth=self._auth)
            if len(OVCall.errors) > 0:
                raise Exception(OVCall.errors)

        def addNewAttempt(self, notifQueueRecId, errorMessage):
            URL = "http://{URL}/api/internal/notif/queue/{notifQueueRecId}/attempts?error_code={errorMessage}".format(URL=self._URL, notifQueueRecId=notifQueueRecId, errorMessage=errorMessage)
...
class IntegrationLog(object):
    ...
    def add(self, logLevel, message, description=""):
        if logLevel.logLevelId <= self._ovLogLevel.logLevelId:
            ...
            headers = {'content-type': 'application/json'}
            url_log = "http://{URL}/api/v3/integrations/runs/{ProcessID}/logs".format(URL=self._URL, ProcessID=self._processId)
