...
function usage() {
    ...
    echo "Usage: $(basename "$0") <website> <version> <owner_schema_username> <owner_schema_password> <user_schema_password> <pkg_schema_password> <rpt_schema_password> <connect_identifier> [platform_edition] [aes_password]"
    echo " "
...
DB_RPT_PASSWORD=$7
DB_URL=$8
PLATFORM_EDITION="$(echo "$9" | tr '[:lower:]' '[:upper:]')"
AES_PASSWORD=${10}
...
"$(dirname "$0")/setup/update-xml-value.py" "$CONTEXT_XML_FILE" 'Parameter[@name="web.dbRpt"]' value "${DB_OWNER_USER}_rpt" || exit 1
"$(dirname "$0")/setup/update-xml-value.py" "$CONTEXT_XML_FILE" 'Parameter[@name="web.dbRptPassword"]' value "$DB_RPT_PASSWORD" || exit 1
